import img1 from "./images/Produits/Hellerman/prod1.jpg";
import img2 from "./images/Produits/Hellerman/prod2.jpg";
import img22 from "./images/Produits/Hellerman/prod22.jpg";
import img3 from "./images/Produits/KCC/prod3.jpg";
import img4 from "./images/Produits/KCC/prod4.jpg";
import img5 from "./images/Produits/KCC/prod5.jpg";
import img6 from "./images/Produits/Lista/prod6.jpg";
import img9 from "./images/Produits/Sylvac/prod9.png";
import img10 from "./images/Produits/Sylvac/prod10.png";
import img11 from "./images/Produits/Sylvac/prod11.png";
import img13 from "./images/Produits/TB/prod13.jpg";
import img14 from "./images/Produits/TB/prod14.jpg";
import img15 from "./images/Produits/TB/prod15.jpg";
import img16 from "./images/Produits/TB/prod16.jpg";
import img17 from "./images/Produits/TB/prod17.jpg";
import img18 from "./images/Produits/Tesa/prod18.png";
import img19 from "./images/Produits/Tesa/prod19.png";
import img20 from "./images/Produits/Tesa/prod20.png";
import img21 from "./images/Produits/Lakes/prod21.png";

import part1 from "./images/Partenaires/HLMT.png";
import part2 from "./images/Partenaires/TESA.png";
import part3 from "./images/Partenaires/KIM.png";
import part4 from "./images/Partenaires/SYLVAC.png";
import part5 from "./images/Partenaires/KSTOOL.png";
import part6 from "./images/Partenaires/WESAG.png";
import part7 from "./images/Partenaires/LAKES.svg";
import part8 from "./images/Partenaires/VALENTIN.svg";
import part9 from "./images/Partenaires/MIBOSTAHL.png";
import part10 from "./images/Partenaires/LISTA.svg";
import part11 from "./images/Partenaires/BIMOS.png";
import part12 from "./images/Partenaires/TB.jpg";
import part13 from "./images/Partenaires/METZNER.png";
import part14 from "./images/Partenaires/RAMATECH.svg";
import part15 from "./images/Partenaires/OES.png";
import part16 from "./images/Partenaires/ARTOS.png";
import part17 from "./images/Partenaires/DINO.jpg";

import staff1 from "./images/QuiSommesNous/founder.jpg";
import staff2 from "./images/QuiSommesNous/ingénieur.jpg";
import staff3 from "./images/QuiSommesNous/sales.jpg";
import staff4 from "./images/QuiSommesNous/responsable.jpg";
import staff5 from "./images/QuiSommesNous/aide-comptable.jpg";
import staff6 from "./images/QuiSommesNous/res-opérations.jpg";
import staff7 from "./images/QuiSommesNous/resp-recouvrement.jpg";
import staff8 from "./images/QuiSommesNous/resp-stock.jpg";
import staff9 from "./images/QuiSommesNous/resp-technicoCommercial.jpg"
import staff10 from "./images/QuiSommesNous/chef-comptable.jpg"
import staff11 from "./images/QuiSommesNous/supply.jpg"
import staff12 from "./images/QuiSommesNous/training.jpg"
import staff13 from "./images/QuiSommesNous/backOffice.jpg"

import prod1 from './images/TopProduits/prod1.png'
import prod2 from './images/TopProduits/prod2.jpg'
import prod3 from './images/TopProduits/prod3.png'
import prod4 from './images/TopProduits/prod4.png'
import prod5 from './images/TopProduits/prod5.png'
import prod6 from './images/TopProduits/prod6.png'
import prod7 from './images/TopProduits/prod7.jpg'
import prod8 from './images/TopProduits/prod8.png'
import prod9 from './images/TopProduits/prod9.jpg'
import prod10 from './images/TopProduits/prod10.jpg'
import partner1 from './images/TopProduits/part1.png'
import partner2 from './images/TopProduits/part2.png'
import partner3 from './images/TopProduits/part3.png'
import partner4 from './images/TopProduits/part4.png'
import partner5 from './images/TopProduits/part5.png'
import partner6 from './images/TopProduits/part6.png'

import accueil1 from './images/Accueil/1.jpg'
import accueil2 from './images/Accueil/2.jpg'
import accueil3 from './images/Accueil/3.jpg'

export const Products = [
  {
    image: img1,
    title: `Pistolet EVO7i`,
    conciseDesc: `EVO 7i offre un tendeur et une coupe précises des colliers de serrage HellermannTyton tout en réduisant les risques liés au travail pour la qualité, la productivité et la sécurité des employés.`,
    fullDesc: `EVO 7i offre un tendeur et une coupe précises des colliers de 
    serrage HellermannTyton tout en réduisant les risques liés au travail 
    pour la qualité, la productivité et la sécurité des employés. Pour une 
    utilisation dans les applications OEM (c'est-à-dire les camions et 
    équipements lourds, les produits blancs, l'automobile), l'assemblage 
    de faisceaux de câbles et la construction de panneaux de commande.`,
    link: "https://www.hellermanntyton.com/products/application-tooling-for-cable-ties/evo7i/110-77001",
    supplier: "HellermannTyton",
  },
  {
    image: img22,
    title: `CPK Hybrid`,
    conciseDesc: `L'autotool CPK hybrid T18RA est un outil automatique de pose pour colliers fonctionnant soit 
    sur secteur soit sur batterie. Il a été développé pour accélérer le processus de mise en faisceaux .`,
    fullDesc: `L'autotool CPK hybrid T18RA est un outil automatique de pose pour colliers fonctionnant soit 
    sur secteur soit sur batterie. Il a été développé pour accélérer le processus de mise en faisceaux 
    et pour garantir une pose de colliers constante, fiable et efficace. Son ergonomie, sa flexibilité 
    et sa facilité d'utilisation en font une solution adaptée aux secteurs solaire, de l’automobile, de 
    l'emballage, des câbliers en général et pour toutes applications de frettage à haute cadence. Le 
    CPK hybrid dispose d'un écran pour modifier les réglages, à commencer par la langue parmi 
    les 20 disponibles.`,
    link: `https://www.hellermanntyton.fr/produits/frettageautomatique/autotool_cpk_hybrid/106-60000`,
    supplier: `HellermannTyton`,
  },
  {
    image: img2,
    title: `Colliers de serrage`,
    conciseDesc: `Colliers de serrage série T pour le regroupement et la fixation de 
    câbles, tuyaux et flexibles. Ces serre-câbles peuvent être utilisés 
    pour presque tous les types d'applications.`,
    fullDesc: `Colliers de serrage série T pour le regroupement et la fixation de 
    câbles, tuyaux et flexibles. Ces serre-câbles peuvent être utilisés 
    pour presque tous les types d'applications.`,
    link: `https://www.hellermanntyton.com/products/cable-ties-inside-serrated/t50r/111-01564`,
    supplier: `HellermannTyton`,
  },
  {
    image: img3,
    title: ` WypAll 8347`,
    conciseDesc: `La gamme WypAll® X80 a été spécialement conçue pour 
    l'essuyage intensif des surfaces rugueuses, où la résistance et le 
    volume sont des exigences clés.`,
    fullDesc: `La gamme WypAll® X80 a été spécialement conçue pour 
    l'essuyage intensif des surfaces rugueuses, où la résistance et le 
    volume sont des exigences clés.
    --
    Les chiffons WypAll® X80 ne se déchirent pas et conservent une 
    qualité et une taille constantes. 70% des utilisateurs ont été 
    impressionnés par la durabilité du tissu, ce qui les a conduits à être 
    préférés aux chiffons par 3 fois plus d'utilisateurs. Leader en force, 
    notre objectif est de vous aider à améliorer votre productivité et 
    votre efficacité tout en réduisant les déchets en fournissant un 
    chiffon propre, fiable et à usage prolongé. La technologie avancée 
    Hydroknit garantit que chaque tissu est solide, durable et très 
    absorbant sans se déchirer. Parfait pour une utilisation dans la 
    fabrication industrielle. Fourni dans un rouleau de grande capacité 
    et lorsqu'il est utilisé avec le distributeur de rouleau mobile, fournit 
    le produit là où vous en avez besoin. Taille de la feuille individuelle 
    34 cm (L) x 31,5 cm (l). Compatible avec le Kimberly-Clark 
    Professional™ (codes produit 6154, 6155 et 6146). 1 grand rouleau 
    x 475 chiffons bleus, 1 épaisseur (475 au total).`,
    link: `https://www.kcprofessional.co.za/products/wiping/repeat-use-cloth-wiping/wypall-x80-cloths-large-roll-steel-blue-8347`,
    supplier: `kimberly clark`,
  },
  {
    image: img4,
    title: `7565`,
    conciseDesc: `Un chiffon à usage répété avec la technologie HYDROKNIT* qui 
    est très absorbant, solide et durable (humide ou sec), résiste à la 
    plupart des solvants, réduit les résidus, permet un nettoyage rapide 
    et aide à réduire les coûts.`,
    fullDesc: `Un chiffon à usage répété avec la technologie HYDROKNIT* qui 
    est très absorbant, solide et durable (humide ou sec), résiste à la 
    plupart des solvants, réduit les résidus, permet un nettoyage rapide 
    et aide à réduire les coûts.
     --
    Idéal pour : les zones industrielles où la séparation des tâches 
    lourdes est requise ; un nettoyage plus rigoureux dans les cuisines et 
    les zones de transformation des aliments ; aider à réduire la contamination croisée en utilisant un code couleur. Les chiffons 
    peuvent être rincés et réutilisés, ce qui contribue à réduire les 
    déchets et à réduire les coûts d'utilisation.
    Disponible sous forme de : essuie-glaces pliés, emballés dans un 
    poly-sac étanche aux éclaboussures et hydrofuge, vous donnant un 
    accès immédiat à des essuie-glaces hygiéniquement protégés dans 
    votre zone de travail.`,
    link: `https://www.kcprofessional.co.uk/en-gb/products/wiping-and-cleaning/disinfecting-and-enhanced-hygiene/colour-coded-cleaning-cloths/7565/7565`,
    supplier: `kimberly clark`,
  },
  {
    image: img5,
    title: `Essuie-mains 6785`,
    conciseDesc: `Essuie-mains en rouleaux de grande capacité qui contrôlent la 
    quantité de mouchoirs que vous prenez et aident à réduire les coûts 
    et le gaspillage.`,
    fullDesc: `Essuie-mains en rouleaux de grande capacité qui contrôlent la 
    quantité de mouchoirs que vous prenez et aident à réduire les coûts 
    et le gaspillage.
     --
    Idéal pour : les zones de contact alimentaire et les toilettes de 
    bureau et d'hôtel très fréquentées qui ont une image de haute qualité 
    ; fournir une sensation de luxe ; séchez vos mains rapidement et 
    confortablement avec moins de serviettes, car elles sont solides et 
    très absorbantes sans se casser lorsqu'elles sont mouillées ; réduire 
    les coûts d'entretien, avec des remplissages moins fréquents.
    Disponible en rouleaux de 130 m d'essuie-mains 1 épaisseur blancs 
    et doux fabriqués à partir de tissu AIRFLEX* solide et très 
    absorbant, sans danger pour un usage alimentaire et certifié FSC.`,
    link: `https://www.olayankcprofessional.com/products/hand-towels/towels/rolled-hand-towel/kleenex-hand-towels-roll-white-6785`,
    supplier: `kimberly clark`,
  },
  {
    image: img6,
    title: ` Armoires à tiroirs`,
    conciseDesc: `Les armoires à tiroirs LISTA offrent une solution de stockage stable 
    et protégée de manière optimale pour toutes sortes de moyens de 
    production et de marchandises.`,
    fullDesc: `Les armoires à tiroirs LISTA offrent une solution de stockage stable 
    et protégée de manière optimale pour toutes sortes de moyens de 
    production et de marchandises.`,
    link: `https://www.lista.com`,
    supplier: `Lista`,
  },
  {
    image: img9,
    title: `Pied à coulisse`,
    conciseDesc: `  Nos pieds à coulisse à affichage numérique offrent une lecture rapide au micromètre. Ils 
    permettent d'effectuer des mesures de cotes intérieures (mesures de fond de gorge, de 
    chanfreins) `,
    fullDesc: `Nos pieds à coulisse à affichage numérique offrent une lecture rapide au micromètre. Ils 
    permettent d'effectuer des mesures de cotes intérieures (mesures de fond de gorge, de 
    chanfreins) et extérieures (épaisseurs, diamètres, cotes variées) sur une large plage de mesure`,
    link: `https://www.sylvac.ch/products/calipers/caliper-s_cal-evo-smart`,
    supplier: `Sylvac`,
  },
  {
    image: img10,
    title: `Comparateur numérique`,
    conciseDesc: `Comparateur numérique adaptatif (jusqu'à 0,2 μm par segment)
    , Version spéciale pour jauges d'alésage cylindrique
    , Introduction de la valeur prédéfinie sur la valeur Min enregistrée
    `,
    fullDesc: `Comparateur numérique adaptatif (jusqu'à 0,2 μm par segment) , Version spéciale pour jauges 
    d'alésage cylindrique , Introduction de la valeur prédéfinie sur la valeur Min enregistrée , 
    Mesure rapide du point d'inversion , Construction robuste , Résistant à l'eau et aux liquides de 
    refroidissement , Technologie Bluetooth® , Nouvelle technologie avec menus configurables , 
    Large choix de fonctions, Mode réveil et veille automatique (Absolute System SIS) , Erreur 
    maximale de 3 μm.`,
    link: `https://www.sylvac.ch/products/digital-indicators/digital-indicator-s_dial-work-cbg`,
    supplier: `Sylvac`,
  },
  {
    image: img11,
    title: `Micromètre`,
    conciseDesc: `Un micromètre numérique
   , Enclumes en forme de couteau 0,75x6,5 mm
   , Connexion de proximité, disponible également avec la technologie 
   Bluetooth® (0-25 mm uniquement)`,
    fullDesc: `
   , Enclumes en forme de couteau 0,75x6,5 mm
   , Connexion de proximité, disponible également avec la technologie 
   Bluetooth® (0-25 mm uniquement)
   , Plage de mesure 0-25 mm / 25-60 mm / 60-95 mm / 95-130 mm / 
   120-155mm
   , Réveil automatique -montée en déplaçant la broche de mesure 
   (Système Absolu SIS)
   , Mode veille après 20 min. d'inutilité (Système Absolu SIS)
   , Position mémorisée en veille (Système Absolu SIS)
   , Etanche pour les travaux lourds avec liquides de refroidissement et 
   lubrifiants, indice de protection IP67 selon IEC 60529, même 
   branché
   , Déplacement rapide de la broche de mesure 10 mm / tour
   , Force de mesure réglable 5N / 10N
   , Répétabilité et reproductibilité jamais atteintes auparavant
   , Cosse en polyacétal-copolymère`,
    link: `https://www.sylvac.ch/products/micrometers/micrometer-s_mike-pro-knife`,
    supplier: `Sylvac`,
  },
  {
    image: img13,
    title: `220P`,
    conciseDesc: `Nylon (polyamide) Une fibre synthétique qui offre une haute 
    résistance à l'abrasion.
    Ne se déforme pas et sèche rapidement.
    Le sans couture permet un meilleur confort et évite les irritations.`,
    fullDesc: `Nylon (polyamide) Une fibre synthétique qui offre une haute 
    résistance à l'abrasion.
    Ne se déforme pas et sèche rapidement.
    Le sans couture permet un meilleur confort et évite les irritations.
    Embouts en PVC pour permettre une meilleure adhérence et une 
    protection contre l'abrasion.`,
    link: `https://tomasbodero.com/en/tomas-bodero/tb-plus/220p.html`,
    supplier: `Tomàs Bodero`,
  },
  {
    image: img14,
    title: `500NG2P URETAN`,
    conciseDesc: `Polyester, une fibre synthétique avec une grande résistance à 
    l'abrasion.
    Moins d'élasticité et d'absorption de la transpiration que le 
    polyamide.`,
    fullDesc: `Polyester, une fibre synthétique avec une grande résistance à 
    l'abrasion.
    Moins d'élasticité et d'absorption de la transpiration que le 
    polyamide.
    Le maintien sans couture apporte du confort et évite les irritations`,
    link: `https://tomasbodero.com/en/tomas-bodero/tb-plus/500ng2p-uretan.html`,
    supplier: `Tomàs Bodero`,
  },
  {
    image: img15,
    title: `700 MF TOUCH`,
    conciseDesc: `Nitrile : caoutchouc qui apporte souplesse et ductilité.
    La poignée Nitrile Sandy permet la ventilation.
    Résistant à l'huile et aux lubrifiants.
    Fibre de nylon très résistante à l'abrasion.`,
    fullDesc: `Nitrile : caoutchouc qui apporte souplesse et ductilité.
    La poignée Nitrile Sandy permet la ventilation.
    Résistant à l'huile et aux lubrifiants.
    Fibre de nylon très résistante à l'abrasion.
    Le support sans couture offre une ergonomie et prévient la fatigue.
    Nitrile de haute qualité.`,
    link: `https://tomasbodero.com/en/tomas-bodero/tb-10-plus/700mf-touch.html`,
    supplier: `Tomàs Bodero`,
  },
  {
    image: img16,
    title: `INDRA S3`,
    conciseDesc: `Chaussure de sécurité : Tissu multicouche hydrofuge haute 
    résistance de couleur gris-noir et cuir suédé noir.
    Embout : fibre de verre non métallique résistant aux chocs 200 J.`,
    fullDesc: `Chaussure de sécurité : Tissu multicouche hydrofuge haute 
    résistance de couleur gris-noir et cuir suédé noir.
    Doublure : tissu Shoe Tech.
    Embout : fibre de verre non métallique résistant aux chocs 200 J.
    Semelle intérieure résistante à la pénétration : non métallique « zéro 
    pénétration » > 1100N.
    Semelle de propreté confort : polyuréthane hermoformant.
    Semelle extérieure : Polyuréthane double densité antistatique.
    ESD`,
    link: `https://tomasbodero.com/en/beework/b-life/indra-s3-grey.html`,
    supplier: `Tomàs Bodero`,
  },
  {
    image: img17,
    title: `NA60`,
    conciseDesc: `Revêtement en caoutchouc nitrile qui présente une bonne résistance 
    aux huiles, lubrifiants et sous-produits pétroliers.
    Résistance aux produits chimiques, aux acides et non aux solvants 
    cétoniques.`,
    fullDesc: `Revêtement en caoutchouc nitrile qui présente une bonne résistance 
    aux huiles, lubrifiants et sous-produits pétroliers.
    Résistance aux produits chimiques, aux acides et non aux solvants 
    cétoniques.
    Protection contre la contamination en cas de contact avec les 
    aliments.
    Imperméable / Ambidextre / Non stérile / NQA 1,5.
    Intérieur non poudré pour éviter les allergies.
    Acide sulfurique 30%-6.
    Dextérité 5 (minimum 0, maximum 5)`,
    link: `https://tomasbodero.com/en/dermik/nitrilo-gloves/na60.html`,
    supplier: `Tomàs Bodero`,
  },
  {
    image: img18,
    title: `4323`,
    conciseDesc: `Ruban de masquage à usage général.
    --
    tesa® 4323 est un adhésif de masquage papier finement crêpé avec 
    une masse adhésive en caoutchouc naturel.`,
    fullDesc: `Ruban de masquage à usage général.
    --
    tesa® 4323 est un adhésif de masquage papier finement crêpé avec 
    une masse adhésive en caoutchouc naturel.`,
    link: `https://www.tesa.com/fr-fr/industrie/tesa-professional-4323.html`,
    supplier: `Tesa`,
  },
  {
    image: img19,
    title: `51025 PV10`,
    conciseDesc: `Toile PET déchirable à la main résistant à l'abrasion et à la 
    température pour le frettage de câbles.`,
    fullDesc: `Toile PET déchirable à la main résistant à l'abrasion et à la 
    température pour le frettage de câbles.
     --
    tesa® 51025 est la meilleure solution économique pour protéger les 
    câbles dans les zones à forte abrasion et température élevée 
    (compartiment moteur et habitacle).
    Caractéristiques :
    • Déchirable à la main
    • Résistant à l'abrasion
    • Résistant à la température 125°C/3000h
    • Résistant aux huiles et à l'essence
    • Ignifuge
    • Sans halogène
    • Résistant au vieillissement
    • Résistant à la moisissure
    • Fort pouvoir adhésif
    • Souple et lisse`,
    link: `https://www.tesa.com/fr-fr/industrie/tesa-51025-pv10.html`,
    supplier: `Tesa`,
  },
  {
    image: img20,
    title: `60760 PV1`,
    conciseDesc: `Ruban d’avertissement destiné au marquage temporaire : bonne 
    puissance adhésive, facile à utiliser, adhérence sur de nombreuses 
    surfaces.`,
    fullDesc: `Ruban d’avertissement destiné au marquage temporaire : bonne 
    puissance adhésive, facile à utiliser, adhérence sur de nombreuses 
    surfaces.
    --
    Le tesa® 60760 est un ruban d’avertissement destiné à un usage 
    temporaire. Alors que le substrat est en PVC souple, l’adhésif 
    modifié est quant à lui composé de caoutchouc naturel afin de 
    garantir une adhérence puissante et de qualité sur la surface 
    concernée. Le ruban d’avertissement résistant à l’abrasion et 
    déchirable à la main peut également être appliqué sur des surfaces 
    rugueuses. En outre, il s’agit d’un outil efficace pour mettre en 
    évidence des dangers présents sur des chantiers de constructions ou 
    dans des ateliers.
    `,
    link: `https://www.tesa.com/fr￾fr/artisans-professionnels/tesa-professional-60760-pv1.html`,
    supplier: `Tesa`,
  },
  {
    image: img21,
    title: `LPI`,
    conciseDesc: `Lakes Precision propose des lames de coupe et de dénudage de fil 
    pour les machines Komax, Schleuniger, Makfil, Megomat, Shäfer, 
    Metzner, Ideal, ARTOS, Eraser et Carpenter etc`,
    fullDesc: `Lakes Precision propose des lames de coupe et de dénudage de fil 
    pour les machines Komax, Schleuniger, Makfil, Megomat, Shäfer, 
    Metzner, Ideal, ARTOS, Eraser et Carpenter ainsi que pratiquement 
    toutes les autres machines de traitement de fil.`,
    link: `https://www.lakesprecision.com/`,
    supplier: `Lakes Precision`,
  },
];
export const Partners = [
  {
    width: "500",
    image: part1,
    url: "https://www.hellermanntyton.com/",
  },
  {
    width: "450",
    image: part2,
    url: "https://www.tesa.com/fr-fr",
  },
  {
    width: "250",
    image: part3,
    url: "https://www.kimberly-clark.com/de-de/germany",
  },
  {
    width: "300",
    image: part4,
    url: "https://www.sylvac.ch/",
  },
  {
    width: "200",
    image: part5,
    url: "https://www.kstools.fr/",
  },
  {
    width: "300",
    image: part6,
    url: "https://www.wezag.de/",
  },
  {
    width: "500",
    image: part7,
    url: "https://www.lakesprecision.com/",
  },
  {
    width: "400",
    image: part8,
    url: "https://www.carl-valentin.de/startseite/",
  },
  {
    width: "400",
    image: part9,
    url: "https://mibostahl.com/fr/",
  },
  {
    width: "300",
    image: part10,
    url: "https://www.lista.com/fr/",
  },
  {
    width: "200",
    image: part11,
    url: "https://www.bimos.com/B/fr-fr/index.php",
  },
  {
    width: "400",
    image: part12,
    url: "https://tomasbodero.com/es/",
  },
  {
    width: "400",
    image: part13,
    url: "https://metzner.com/index.php/de/",
  },
  {
    width: "400",
    image: part14,
    url: "https://www.ramatech.ch/index.php/de/",
  },
  {
    width: "300",
    image: part15,
    url: "https://www.oes-inc.com/",
  },
  {
    width: "300",
    image: part16,
    url: "https://www.artosengineering.com/",
  },
  {
    width: "200",
    image: part17,
    url: "https://www.dino-lite.com/",
  }
];
export const StaffsIMG = [
  {
    name: "Abdessalem BARKIA",
    post: "Directeur Général",
    email: "a.barkia@tch-industries.com",
    linkedin: "https://tn.linkedin.com/in/abdessalembarkia",
    image: staff1,
  },
  {
    name: "Hichem NAHALI",
    post: "ING. Technico-Commercial ",
    email: "h.nahali@tch-industries.com",
    linkedin: "https://www.linkedin.com/in/hichem-nahali-5ba791124/",
    image: staff2,
  },
  {
    name: "M’Hammed M’SAAD",
    post: "ING. Technico-Commercial",
    email: "m.msaad@tch-industries.com ",
    linkedin: "https://tn.linkedin.com/in/m-hammed-m-saad",
    image: staff3,
  },
  {
    name: "Mariem CHBINOU",
    post: "Resp. Administratif / Financier",
    email: "m.chbinou@tch-industries.com",
    linkedin: "https://www.linkedin.com/in/mariem-chbinou-33a1a2148/",
    image: staff4,
  },
  {
    name: "Rimeh CHARAABI",
    post: "Aide Comptable",
    email: "r.charaabin@tch-industries.com",
    linkedin: "https://www.linkedin.com/in/rimeh-charaabi/",
    image: staff5,
  },{
    name: "Haykel SOMRANI",
    post: "Resp. des opérations",
    email: "h.somrani@tch-industries.com",
    linkedin: "https://www.linkedin.com/in/haykel-somrani-63999a186/",
    image: staff6,
  },{
    name: "Jihen BEN MAHMOUD",
    post: "Resp. Recouvrement",
    email: "j.benmahmoud@tch-industries.com",
    linkedin: "https://www.linkedin.com/in/jihen-ben-mahmoud-92140b247/",
    image: staff7,
  },{
    name: "Marouen ZAHROUNI",
    post: "Resp. Stock",
    email: "magasin@tch-industries.com",
    linkedin: "https://www.linkedin.com/in/zahrouni-marwen-b6b675219/",
    image: staff8,
  },{
    name: "Ahmed BARHOUMI",
    post: "Représentant technico-commercial",
    email: "a.barhoumi@tch-industries.com",
    linkedin: "https://www.linkedin.com/in/barhoumi-ahmed-3b38b0237/",
    image: staff9,
  },
  {
    name: "Marwa BEN SLIMEN",
    post: "Chef Comptable",
    email: "m.benslimen@tch-industries.com",
    linkedin: "https://www.linkedin.com/in/marwa-benslimen-3bab08206/",
    image: staff10,
  },{
    name: "Khouloud KHALFAOUI",
    post: "Resp. Achat / Logistique ",
    email: "k.khalfaoui@tch-industries.com",
    linkedin: "https://www.linkedin.com/in/khouloud-khalfaoui-563a248b/",
    image: staff11,
  },{
    name: "Manel BEN AMARA",
    post: "Resp. Formation",
    email: "m.benamara@tch-industries.com",
    linkedin: "https://www.linkedin.com/in/manel-ben-amara-22425a215/",
    image: staff12,
  },{
    name: "Chedly BEN MAHMOUD",
    post: "Gestionnaire Back Office ",
    email: "c.benmahmoud@tch-industries.com",
    linkedin: "https://www.linkedin.com/in/ben-mahmoud-chedly-706b42220/",
    image: staff13,
  },
];

export const Prod = [
  {
    image1: prod1,
    image2: partner1,
    partLink: 'https://www.tesa.com/fr-fr'
  },
  {
    image1: prod2,
    image2: partner2,
    partLink: 'https://www.hellermanntyton.com/'
  },
  {
    image1: prod3,
    image2: partner6,
    partLink: 'https://www.lakesprecision.com/'
  },
  {
    image1: prod4,
    image2: partner4,
    partLink: 'https://www.sylvac.ch/'
  },
  {
    image1: prod5,
    image2: partner3,
    partLink: 'https://metzner.com/index.php/en/'
  },
  {
    image1: prod6,
    image2: partner1,
    partLink: 'https://www.tesa.com/fr-fr'
  },
  {
    image1: prod7,
    image2: partner2,
    partLink: 'https://www.hellermanntyton.com/'
  },
  {
    image1: prod8,
    image2: partner6,
    partLink: 'https://www.lakesprecision.com/'
  },
  {
    image1: prod9,
    image2: partner5,
    partLink: 'https://www.lista.com/fr/'
  },
  {
    image1: prod10,
    image2: partner3,
    partLink: 'https://metzner.com/index.php/en/'
  },
];

export const Sliders = [{
    name: 'random',
    description: 'random',
    image: accueil1

  },{
    name: 'random',
    description: 'random',
    image: accueil2

  },{
    name: 'random',
    description: 'random',
    image: accueil3

  }]
